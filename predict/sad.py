...
# 1. 輸出目錄與檔案路徑設定（悲傷專用）
# ==============================================================
OUTPUT_DIR = r"C:\Users\user\Desktop\output\第一次最終分類器資料\classifier\sad"
os.makedirs(OUTPUT_DIR, exist_ok=True)
MODEL_PATH = os.path.join(OUTPUT_DIR, "三模態悲傷情緒分類器_最終模型.pkl")
GT_CSV = os.path.join(OUTPUT_DIR, "每秒人工標註悲傷真值.csv")
TIMELINE_CSV = os.path.join(OUTPUT_DIR, "每秒預測悲傷機率與真值.csv")
RESULT_PLOT = os.path.join(OUTPUT_DIR, "圖4.9_三模態悲傷情緒預測與人工真值對比.png")
IMPORTANCE_CSV = os.path.join(OUTPUT_DIR, "悲傷特徵重要性排名_TOP20.csv")
PERFORMANCE_TXT = os.path.join(OUTPUT_DIR, "悲傷模型績效指標總表.txt")
...
print("《復仇者聯盟：終局之戰》三模態悲傷（Sad）情緒辨識系統 —— 最終可信版本")
print("="*78)
# ==============================================================
# 2. 產生每秒人工標註「悲傷」真值
# ==============================================================
print("步驟 1：讀取人工標註資料並生成每秒悲傷真值（Ground Truth）")
...
悲傷關鍵字 = ["悲傷", "傷心", "難過", "哭", "哭泣", "淚水", "失落", "心痛", "絕望", "沮喪",
             "Sad", "sad", "cry", "tears", "depressed", "heartbroken", "sorrow", "mourn"]
for _, row in df_ann.iterrows():
    try:
        s = int(time_to_sec(row["Start Time (開始時間)"]))
        e = int(time_to_sec(row["End Time (結束時間)"]))
        label = str(row["Label (主標籤)"]).lower()
        if any(kw.lower() in label for kw in 悲傷關鍵字):
            gt[s:e+1] = 1
    except:
        continue
gt_df = pd.DataFrame({"second": range(total_seconds), "sad_gt": gt})
gt_df.to_csv(GT_CSV, index=False, encoding="utf-8-sig")
sad_ratio = gt.sum() / total_seconds * 100
print(f" 完成：人工標註悲傷時長 {gt.sum()//60} 分 {gt.sum()%60} 秒（占全片 {sad_ratio:.2f}%）")
# ==============================================================
# 3. 建置三模態物理特徵資料庫
# ==============================================================
...
df_final["sad_gt"] = df_final["sad_gt"].fillna(0).astype(int)
# ==============================================================
# 4. 特徵工程
# ==============================================================
...
numeric_cols = df_final.select_dtypes(include=[np.number]).columns.drop(["second","sad_gt"], errors='ignore')
...
X = df_final.drop(columns=["second", "sad_gt"])
y = df_final["sad_gt"]
# ==============================================================
...
if isinstance(best_model, nn.Module):
    ...
    with torch.no_grad():
        ...
        df_final["sad_prob"] = best_model(X_all_tensor).cpu().numpy().flatten()
else:
    df_final["sad_prob"] = best_model.predict_proba(X_all_scaled)[:,1]
df_final[["second", "sad_prob", "sad_gt"]].to_csv(TIMELINE_CSV, index=False, encoding="utf-8-sig")
...
with open(PERFORMANCE_TXT, "w", encoding="utf-8") as f:
    f.write("《復仇者聯盟：終局之戰》三模態悲傷（Sad）情緒辨識系統\n")
    f.write("="*72 + "\n")
    f.write("最終模型績效指標總表（適合學術發表）\n")
    f.write("="*72 + "\n\n")
    f.write(f"資料概況\n")
    f.write(f" 影片總長度 ：169 分 10 秒（{total_seconds:,} 秒）\n")
    f.write(f" 人工標註悲傷時長 ：{int(gt.sum())//60:3d} 分 {int(gt.sum())%60:02d} 秒（{sad_ratio:.2f}%）\n")
...
joblib.dump({
    ...
    "emotion": "sad", "auc": final_test_auc
}, MODEL_PATH)
plt.figure(figsize=(24,9))
time_min = df_final["second"]/60.0
plt.plot(time_min, df_final["sad_prob"], label="三模態模型預測悲傷機率", color="#F44336", linewidth=2.8)
plt.fill_between(time_min, 0, df_final["sad_gt"], color="#D32F2F", alpha=0.6, label="人工標註悲傷區間")
plt.title("《復仇者聯盟：終局之戰》三模態悲傷情緒辨識結果\n與人工真值對比（圖4.9）", fontsize=22, pad=30)
plt.xlabel("時間（分鐘）", fontsize=18)
plt.ylabel("悲傷機率", fontsize=18)
