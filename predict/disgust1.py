# -*- coding: utf-8 -*-
"""
《復仇者聯盟：終局之戰》三模態厭惡（Disgust）情緒辨識系統
...
# 1. 輸出目錄與檔案路徑設定（厭惡專用）
# ==============================================================
OUTPUT_DIR = r"C:\Users\user\Desktop\output\第一次最終分類器資料\classifier\disgust"
os.makedirs(OUTPUT_DIR, exist_ok=True)
MODEL_PATH = os.path.join(OUTPUT_DIR, "三模態厭惡情緒分類器_最終模型.pkl")
GT_CSV = os.path.join(OUTPUT_DIR, "每秒人工標註厭惡真值.csv")
TIMELINE_CSV = os.path.join(OUTPUT_DIR, "每秒預測厭惡機率與真值.csv")
RESULT_PLOT = os.path.join(OUTPUT_DIR, "圖4.9_三模態厭惡情緒預測與人工真值對比.png")
IMPORTANCE_CSV = os.path.join(OUTPUT_DIR, "厭惡特徵重要性排名_TOP20.csv")
PERFORMANCE_TXT = os.path.join(OUTPUT_DIR, "厭惡模型績效指標總表.txt")
...
print("《復仇者聯盟：終局之戰》三模態厭惡（Disgust）情緒辨識系統 —— 最終可信版本")
print("="*78)

# ==============================================================
# 2. 產生每秒人工標註「厭惡」真值
# ==============================================================
print("步驟 1：讀取人工標註資料並生成每秒厭惡真值（Ground Truth）")
...
厭惡關鍵字 = ["厭惡", "噁心", "反感", "嫌惡", "作嘔", "討厭", "惡心", "嫌棄", "排斥", "鄙視",
             "disgust", "gross", "repulsive", "nausea", "revulsion", "contempt", "sickened", "yuck"]
...
for _, row in df_ann.iterrows():
    try:
        ...
        if any(kw.lower() in label for kw in 厭惡關鍵字):
            gt[s:e+1] = 1
    except:
        continue

gt_df = pd.DataFrame({"second": range(total_seconds), "disgust_gt": gt})
gt_df.to_csv(GT_CSV, index=False, encoding="utf-8-sig")

disgust_ratio = gt.sum() / total_seconds * 100
print(f" 完成：人工標註厭惡時長 {gt.sum()//60} 分 {gt.sum()%60} 秒（占全片 {disgust_ratio:.2f}%）")
...
df_final["disgust_gt"] = df_final["disgust_gt"].fillna(0).astype(int)
...
numeric_cols = df_final.select_dtypes(include=[np.number]).columns.drop(["second","disgust_gt"], errors='ignore')
...
X = df_final.drop(columns=["second", "disgust_gt"])
y = df_final["disgust_gt"]
...
if isinstance(best_model, nn.Module):
    ...
    with torch.no_grad():
        ...
        df_final["disgust_prob"] = best_model(X_all_tensor).cpu().numpy().flatten()
else:
    df_final["disgust_prob"] = best_model.predict_proba(X_all_scaled)[:,1]

df_final[["second", "disgust_prob", "disgust_gt"]].to_csv(TIMELINE_CSV, index=False, encoding="utf-8-sig")
...
with open(PERFORMANCE_TXT, "w", encoding="utf-8") as f:
    f.write("《復仇者聯盟：終局之戰》三模態厭惡（Disgust）情緒辨識系統\n")
    f.write("="*72 + "\n")
    f.write("最終模型績效指標總表（適合學術發表）\n")
    f.write("="*72 + "\n\n")
    f.write(f"資料概況\n")
    f.write(f" 影片總長度 ：169 分 10 秒（{total_seconds:,} 秒）\n")
    f.write(f" 人工標註厭惡時長 ：{int(gt.sum())//60:3d} 分 {int(gt.sum())%60:02d} 秒（{disgust_ratio:.2f}%）\n")
...
joblib.dump({
    ...
    "emotion": "disgust", "auc": final_test_auc
}, MODEL_PATH)

# 繪圖（厭惡用深綠/灰褐色系）
plt.figure(figsize=(24,9))
time_min = df_final["second"]/60.0
plt.plot(time_min, df_final["disgust_prob"], label="三模態模型預測厭惡機率", color="#388E3C", linewidth=2.8)
plt.fill_between(time_min, 0, df_final["disgust_gt"], color="#2E7D32", alpha=0.6, label="人工標註厭惡區間")
plt.title("《復仇者聯盟：終局之戰》三模態厭惡情緒辨識結果\n與人工真值對比（圖4.9）", fontsize=22, pad=30)
plt.xlabel("時間（分鐘）", fontsize=18)
plt.ylabel("厭惡機率", fontsize=18)
